...
public class BringerBattleState : EnemyStates
{
    private Enemy_Bringer enemy;
    private Transform player;
    private int moveDir;

    public BringerBattleState(Enemy _enemyBase, EnemyStateMachine enemyStateMachine, string animBoolName, Enemy_Bringer _enemy) : base(_enemyBase, enemyStateMachine, animBoolName)
    {
        this.enemy = _enemy;
    }
    ...
    public override void Update()
    {
        ...
        if (enemy.IsPlayerDetected() && enemy.IsPlayerDetected().distance < enemy.attackDist - .1f)
            return;
