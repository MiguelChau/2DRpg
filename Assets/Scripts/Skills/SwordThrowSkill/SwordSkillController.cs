...
public class SwordSkillController : MonoBehaviour
{
    ...
    public void SetupSword(Vector2 _dir, float _gravityScale, Player _player, float _freezeTimeDur, float _returnSpeed) //configura a espada com info iniciais
    {
        //Parametros
    ...
    public void SetupPierce(int _piercingAmount)
    {
        pierceAmount = _piercingAmount;
    }

    public void SetupSpin(bool _isSpinning, float _maxTravelDis, float _spinDur, float _hitCooldown)
    {
        isSpinning = _isSpinning;
        maxTravelDis = _maxTravelDis;
        spinDur = _spinDur;
    ...
    private void Update()
    {
        if (canRotate) //o que vai fazer rodar a espada
    ...
    private void SpinLogic()
    {
        if (isSpinning)
        {
            if (Vector2.Distance(player.transform.position, transform.position) > maxTravelDis && !wasStopped)
            {
                StopWhenSpinning();
            }
